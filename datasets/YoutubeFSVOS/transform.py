...
class ToFloat(object):
    ...
    def __call__(self, imgs, annos):
        ...
        for idx, anno in enumerate(annos):
            ...
            # DON'T divide masks by 255 - YoutubeVOS masks already have values {0, 1}
            # Dividing by 255 would turn them into {0.0, ~0.004} causing zero mIoU
            # annos[idx] /= 255.0
...
class Rescale(object):
    ...
    def __call__(self, imgs, annos):
        ...
        for id, img in enumerate(imgs):
            ...
            # print(f"shape of images: {canvas.shape}")
            imgs[id] = canvas
        ...
        for id, anno in enumerate(annos):
            ...
            # print(f"count of points non zero in anno before rescale: {np.count_nonzero(anno)}")
            # print(f"shape of mask: {canvas.shape}")
            annos[id] = canvas
...
class Stack(object):
    ...
    def __call__(self, imgs, annos):
        ...
        # print(f"shape of img_stack: {img_stack.shape}")
        # print(f"shape of anno_stack: {anno_stack.shape}")
        # print(f"count of points non zero in anno after stacking: {np.count_nonzero(anno_stack)}")
...
class ToTensor(object):
    ...
    def __call__(self, imgs, annos):
        ...
        annos = torch.from_numpy(annos.copy())
...
class ResizeToSquare(object):
    """
    rescale the size of image and masks to square
    """
    ...
    def __call__(self, imgs, annos):
        ...
        for id, img in enumerate(imgs):
            rescaled_img = cv2.resize(img, (new_size[1], new_size[0]), cv2.INTER_LINEAR)
            imgs[id] = rescaled_img

        for id, anno in enumerate(annos):
            rescaled_anno = cv2.resize(anno, (new_size[1], new_size[0]), cv2.INTER_NEAREST)
            annos[id] = rescaled_anno[:,:,np.newaxis]
