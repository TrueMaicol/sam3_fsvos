...
class NMiniVSPWEpisodicData(EpisodicData):
    ...
    Can be instantiated with individual parameters (preferred) or with an args object.
    
    Parameters:
        fold (int): Fold number for train/test split. Default: 0
        sprtset_as_frames (bool): Whether to use support set as frames. Default: False
        split_type (str): Type of split ('train', 'val', 'test'). Default: 'test'
        shot (int): Number of support samples. Default: 1
        data_root (str): Root directory for dataset. Required.
        data_list_path (str): Path to the data list file. Required.
    ...
    def __init__(
        ...
        **kwargs
        ...
        if args is not None:
            ...
            _data_list_path = kwargs.get('data_list_path', data_list_path)
        else:
            # New mode: use individual parameters
            self.fold = fold
        ...
        self.benchmark = 'minivspw'
        # Validate required parameters
        if _data_root is None:
            raise ValueError("data_root is required")
        if _data_list_path is None:
            raise ValueError("data_list_path is required")
        ...
        params_proxy = _ParamsProxy()
        params_proxy.shot = _shot
