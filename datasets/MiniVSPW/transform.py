...
class Compose(object):
    ...
    def __call__(self, image, label=None, flow=None):
        if label is None:
            ...
            for t in self.segtransform:
                image, label, flow = t(image, label, flow=flow)
...
class Resize(object):
    ...
    def __call__(self, image, label, flow=None):
        ...
        if flow is not None:
            flow_crop = cv2.resize(flow, dsize=(int(new_w), int(new_h)),
                                    interpolation=cv2.INTER_LINEAR)
        ...
        if flow is not None:
            ...
            if self.padding:
                ...
                back_crop_flow[:, :, 1] = self.padding[1]

            back_crop_flow[:new_h, :new_w, :] = flow_crop
            flow = back_crop_flow
...
class RandScale(object):
    ...
    def __call__(self, image, label, flow=None):
        ...
        image = cv2.resize(image, None, fx=scale_factor_x, fy=scale_factor_y,
                           interpolation=cv2.INTER_LINEAR)
        if flow is not None:
            flow = cv2.resize(flow, None, fx=scale_factor_x, fy=scale_factor_y,
                               interpolation=cv2.INTER_LINEAR)
...
class Crop(object):
    ...
    def __call__(self, image, label, flow=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            if flow is not None:
                flow = cv2.copyMakeBorder(flow, pad_h_half, pad_h - pad_h_half, pad_w_half,
                                           pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.padding)
        ...
        if flow is not None:
            flow = flow[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
        if label is not None:
...
class RandRotate(object):
    ...
    def __call__(self, image, label, flow=None):
        if random.random() < self.p:
            ...
            if flow is not None:
                flow = cv2.warpAffine(flow, matrix, (w, h), flags=cv2.INTER_LINEAR,
                                       borderMode=cv2.BORDER_CONSTANT, borderValue=self.padding)
